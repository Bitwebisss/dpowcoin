...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def __init__(self) -> None:
        ...
        self.rpc_timeout = 240  # Wait for up to 60 seconds for the RPC server to respond
    ...
    def parse_args(self):
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop dpowcoinds after the test execution")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "dpowcoind": ("dpowcoind", "DPOWCOIND"),
            "dpowcoin-cli": ("dpowcoincli", "DPOWCOINCLI"),
            "dpowcoin-util": ("dpowcoinutil", "DPOWCOINUTIL"),
            "dpowcoin-wallet": ("dpowcoinwallet", "DPOWCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: dpowcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("DpowcoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'dpowcoind', self.options.dpowcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'dpowcoin-cli', self.options.dpowcoincli) for v in versions]
    ...
    def wait_until(self, test_function, timeout=300):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    dpowcoind=self.options.dpowcoind,
                    dpowcoin_cli=self.options.dpowcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
